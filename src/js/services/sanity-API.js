...
export async function fetchAllProductsData() {
	const products = await client.fetch(
		'*[_type == "product"]{nameEN, nameES, isSpicy, category, price, descriptionEN, descriptionES, customUrl, weight, isTopSeller,_id,  "imageUrl": image.asset->url}'
	);
	return products;
}
...
export async function fetchProductByURL(url) {
	const [product] = await client.fetch(
		`*[_type == 'product' && customUrl == '${url}']{nameEN, nameES, isSpicy, category, price, descriptionEN, descriptionES, customUrl, weight, isTopSeller,_id, "imageUrl": image.asset->url}`
	);
	return product;
}
...
export async function fetchProductDataById(id) {
	const product = await client.fetch(
		`*[_type == 'product' && _id == '${id}']{nameEN, nameES, isSpicy, category, price, descriptionEN, descriptionES, customUrl, weight, isTopSeller,_id, "imageUrl": image.asset->url}`
	);
	return product;
}
